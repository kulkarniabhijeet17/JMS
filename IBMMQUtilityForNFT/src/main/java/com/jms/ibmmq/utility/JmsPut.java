...
public class JmsPut {
    private static final Logger logger = Logger.getLogger(JmsPut.class.getName());
    private static int status = 1;

    public static void main(String[] args) {
        String textFilePath = System.getProperty("user.dir") + "/src/main/resources/NFTLogs/" + LocalDate.now();
        String textFileName = "MQ_Tier_Push_Log_" + LocalDate.now();
        ...
        try {
            ...
            // NFT Specifics
            int tps, minutes, seconds;
            tps = Integer.parseInt(prop.getProperty("nft.load.tps"));
            minutes = Integer.parseInt(prop.getProperty("nft.stress.duration.minutes"));
            seconds = Integer.parseInt(prop.getProperty("nft.stress.duration.seconds"));

            long noOfMessages = (long) tps * ((minutes * 60L) + seconds);
            long delay = 0;
            ...
            if (noOfMessages > 0) {
                long i = 0;
                if (noOfMessages > 1) {
                    delay = 1000000000 / tps;

                    // First Message Push
                    currTime = System.nanoTime();
                    futureTime = currTime + delay - 50;
                    ...
                    double totalNanos = (duration.getSeconds() * 1000_000_000) + duration.getNano() + delay;
